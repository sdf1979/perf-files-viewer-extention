...
std::string PerfFilesViewerAddIn::extensionName() {
    return "PerfFilesViewer";
}

PerfFilesViewerAddIn::PerfFilesViewerAddIn() {
    ...
    AddMethod(L"Add", L"Сложить", this, &PerfFilesViewerAddIn::add);
    AddMethod(L"Message", L"Сообщить", this, &PerfFilesViewerAddIn::message);
    AddMethod(L"CurrentDate", L"ТекущаяДата", this, &PerfFilesViewerAddIn::currentDate);
    AddMethod(L"Assign", L"Присвоить", this, &PerfFilesViewerAddIn::assign);
    AddMethod(L"SamplePropertyValue", L"ЗначениеСвойстваОбразца", this, &PerfFilesViewerAddIn::samplePropertyValue);
    AddMethod(L"ExecuteCommand", L"ВыполнитьКоманду", this, &PerfFilesViewerAddIn::executeCommand);
    ...
    //
    AddMethod(L"Sleep", L"Ожидать", this, &PerfFilesViewerAddIn::sleep, {{0, 5}});

    perf_logs_reader_ = std::make_unique<PerfLogsReader>();
}
...
variant_t PerfFilesViewerAddIn::add(const variant_t &a, const variant_t &b) {
...
void PerfFilesViewerAddIn::message(const variant_t &msg) {
...
void PerfFilesViewerAddIn::sleep(const variant_t &delay) {
    using namespace std;
...
void PerfFilesViewerAddIn::assign(variant_t &out) {
    out = true;
}

// Despite that you can return property value through method this is not recommended
// due to unwanted data copying
variant_t PerfFilesViewerAddIn::samplePropertyValue() {
    return *sample_property;
}

variant_t PerfFilesViewerAddIn::currentDate() {
    using namespace std;
    tm current{};
...
variant_t PerfFilesViewerAddIn::executeCommand(const variant_t& cmd) {
    return perf_logs_reader_->executeCommand(std::get<std::string>(cmd));
}